...
**관측:** (2026-07-28, 완료 알림창 동작)

> **완료 알림창은 진행률 0% 일 때 이미 떠 있다.**
>
> `대화 내보내기` 창은 게이지가 차는 동안 계속 떠 있고, 다 차면
> "완료되었습니다" 로 바뀐다. **Enter 만으로 닫힌다** (버튼 클릭 불필요).
>
> 따라서 창을 찾자마자 닫으면 저장이 끝나기 전에 중단된다.
> `wait_file_ready()` 로 **실제 파일이 다 쓰였는지**를 완료 신호로 쓰도록
> 고쳤다. 창의 존재만으로 완료를 판단하지 말 것.
