...
**권한** (2026-08-02 소유자 지시로 갱신)

  - `/start` 한 사람은 누구나 문구를 **받고**, 링크를 보내 **변환도 시킨다**
  - 관리자(`TG_CHAT_ID`/`TG_ADMIN_IDS`)만 구독자 목록(`/subs`)을 본다

  ⚠️ **권한은 열렸지만 속도는 열리지 않았다.** 변환 한 번이 곧 소유자
  PC 의 브라우저로 파트너스·토스에 접속하는 것이라, 사람이 늘면 접근
  빈도도 사람 수만큼 는다. `subscribers.convert_allowed()` 가 총량·1인당·
  간격으로 묶어 CLAUDE.md 제약 2 를 지킨다. 같이 풀지 마라.
