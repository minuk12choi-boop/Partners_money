...
⚠️ **권한은 열렸지만 속도는 열리지 않았다. 이 둘을 같이 풀지 마라.**
변환 한 번이 곧 소유자 PC 의 브라우저로 쿠팡 파트너스 / 토스에 접속하는
것이다. 사람이 열 명이면 접근도 열 배가 되고, 그러면 CLAUDE.md 제약 2
(접근 빈도)가 코드가 아니라 사람 손으로 깨진다. 계정이 정지되면 구독자가
몇 명이든 의미가 없다. `convert_allowed()` 가 총량으로 묶는 이유다.
